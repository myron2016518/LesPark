...
function showMsg(msg) {
    ...
    switch (subType) {
        case webim.GROUP_MSG_SUB_TYPE.COMMON: //群普通消息
            ...
            console.log('群普通消息convertMsgtoHtml，',contentSpan);
            break;
    }
...
function convertMsgtoHtml(msg) {
    console.log('解析群提示消息元素content',msg);
    var html = "",
    ...
    for (var i in elems) {
        ...
        console.log('群提示消息类型opType>>>>>>>>>>>>>>>>', type);
        switch (type) {
...
function onSendMsg(msg,callback) {
    ...
    if (!emotions || emotions.length < 1) {
        text_obj = new webim.Msg.Elem.Custom(msgtosend);
